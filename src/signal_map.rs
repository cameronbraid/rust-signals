...
/// indirection.
pub type BoxSignalMap<'a, Key, Value> =
    Pin<Box<dyn SignalMap<Key = Key, Value = Value> + Send + 'a>>;
...
#[inline]
pub fn always<A, K, V>(map: A) -> Always<A>
where
    A: IntoIterator<Item = (K, V)>,
{
    Always { map: Some(map) }
...
#[derive(Debug)]
#[must_use = "SignalMaps do nothing unless polled"]
pub struct MapValueSignal<A, B, F>
where
    A: SignalMap,
    B: Signal,
{
                ...
                Some(Poll::Ready(Some(change))) => {
                    new_pending.push(match change {
                            ...
                            MapDiff::Replace {
                                entries: entries
                                    ...
                                    .map(|(key, value)| {
                                        ...
                                        let poll =
                                            (key.clone(), unwrap(signal.as_mut().poll_change(cx)));
                        ...
                        MapDiff::Update { key, value } => {
                            let mut signal = Box::pin(callback(value));
                            let poll = unwrap(signal.as_mut().poll_change(cx));
                            *signals.get_mut(&key).expect("The key is not in the map") =
                                Some(signal);
        ...
        for (key, signal) in signals.into_iter() {
            ...
            match signal.as_mut().map(|s| s.as_mut().poll_change(cx)) {
                Some(Poll::Ready(Some(value))) => {
                    new_pending.push(MapDiff::Update {
                        key: key.clone(),
                        value,
                    });
                }
            ...
            return match signal.as_mut().poll_map_change(cx) {
                ...
                Poll::Ready(Some(change)) => match change {
                    MapDiff::Replace { entries } => {
                        ...
                        *forwarded_keys = entries.iter().map(|(k, _v)| (*k).clone()).collect();
                    ...
                    MapDiff::Update { key, value } => {
                        // no need to test the callback here, since we know which keys are being forwarded

                        if callback(&key, &value) {
                            // are still interested in the key OR value

                            if forwarded_keys.contains(&key) {
                                // downstream is aware of this key
                                Poll::Ready(Some(MapDiff::Update { key, value }))
                            } else {
                                // downstream is not aware of this key
                                forwarded_keys.insert(key.clone());
                                Poll::Ready(Some(MapDiff::Insert { key, value}))
                            }
                        } else {

                          // NOT interested in the key OR value

                            if forwarded_keys.contains(&key) {
                                // downstream is aware of this key
                                forwarded_keys.remove(&key);
    ...
    A::Value: Clone,
{
    ...
    fn poll_vec_change(
        ...
        let SignalMapEntriesProj { mut signal, keys } = self.project();
